...
const MobileMenu = () => {
        ...
        return ( 
                <>
                 ...
                 <Image onClick={()=> setVisible(!visible)} src={assets.menu_icon} alt='search-icon' className='w-5 cursor-pointer' />
                 <div className={` top-0 left-0 bottom-0 overflow-hidden bg-white transition-all shadow-xl fixed ${visible ? "w-[60%]" : "w-0"}`}>
                        <div className="flex flex-col text-gray-600">
                                <div onClick={()=> setVisible(false)} className="flex items-center gap-1 p-3 cursor-pointer">
                                        ...
                                        <Image src={assets.dropdown_icon} alt='search-icon' className='w-5 cursor-pointer' />
                                        <p>بازگشت</p>
                                </div>
                                {
                                        navItems.map((item) => (
